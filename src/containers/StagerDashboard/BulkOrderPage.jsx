...
class BulkOrderPage extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  handleloansSubmit() {
    ...
    const { onLoansSubmit, user } = this.props;
    const { onFailedLoanValidation } = this.props;
    if (validateLoanFormat(loansNumber)) {
      const loanNumbersList = loansNumber.trim().replace(/\n/g, ',').split(',').map(s => s.trim());
      const payload = {};
      payload.loanNumber = loanNumbersList;
      payload.eventName = value;
      payload.status = selectedState;
      payload.userPrincipalName = user.userDetails.email;
      ...
      <>
        <ContentHeader title={title}>
          <Grid container style={{ height: '3rem' }} xs={12}>
            <Grid item xs={2}>
              <div styleName="backButton"><Link onClick={this.handleBackButton} to="/stager">&lt; BACK</Link></div>
            </Grid>
...
const mapStateToProps = state => ({
  ...
  user: LoginSelectors.user(state),
});

const mapDispatchToProps = dispatch => ({
  onLoansSubmit: stagerOperations.onLoansSubmit(dispatch),
  onFailedLoanValidation: operations.onFailedLoanValidation(dispatch),
});

const BulkOrderPageContainer = connect(mapStateToProps, mapDispatchToProps)(BulkOrderPage);

const TestHooks = {
  BulkOrderPage,
};

export default withRouter(BulkOrderPageContainer);
export { TestHooks };
