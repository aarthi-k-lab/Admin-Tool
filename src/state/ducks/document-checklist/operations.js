...
import {
  ...
  docValidation, defectReasonDropdown, setSelectedBorrower, fetchDocHistory, resetFileNetDocTypes,
  fetchFilenetCatTypes, fetchFilenetTypes, addContributor,
} from './actions';
...
const resetFileNetDocTypesOperation = dispatch => () => {
  dispatch(resetFileNetDocTypes());
};

const fetchFilenetCatTypesOperation = dispatch => () => {
  dispatch(fetchFilenetCatTypes());
};

const fetchFilenetTypesOperation = dispatch => (payload) => {
  dispatch(fetchFilenetTypes(payload));
};

const addContributorOperation = dispatch => (payload) => {
  dispatch(addContributor(payload));
};
...
export default {
  ...
  resetFileNetDocTypesOperation,
  fetchFilenetCatTypesOperation,
  fetchFilenetTypesOperation,
